...
import assignmenttwo.ConcurrentQueue
...
class JMHSample_01_HelloWorld {
  ...
  def wellHelloThere(): Unit = {
    val q : java.util.concurrent.ConcurrentLinkedQueue[String] = new java.util.concurrent.ConcurrentLinkedQueue[String]()
    for(i <- 0 to 5500) q.offer(i + "")
    val forkOne : Future[Unit] = Future {
      val it = q.iterator
      while (it.hasNext) it.next()
    }
    for (i <- 1 to 5500) q.poll()
    Await.result(forkOne, Duration.Inf)
  }
}
